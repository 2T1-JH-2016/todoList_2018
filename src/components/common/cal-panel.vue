<template>
  <div class="cal-wrapper">
    ...
    <div class="cal-body">
      <div class="weeks">
        <span
          v-for="(dayName, dayIndex) in kor[calendar.options.locale].dayNames"
          class="item"
          :key="dayIndex"
          >
          {{kor[calendar.options.locale].dayNames[(dayIndex + calendar.options.weekStartOn) % 7]}}
        </span>
      </div>
      <div class="dates" >
        <div v-for="date in dayList" class="item"
          ...
          <p class="date-num"
            ...
            :style="{color: date.title != undefined ? ((date.date == selectedDay) ? '#fff' : '#000' ) : 'inherit'}">
          ...
          <span v-if="date.status ? (date.title != undefined) : false" class="is-event"
            :style="{borderColor: 'customColor', backgroundColor: (date.date == selectedDay) ? '#b790b9' : 'inherit'}"></span>
        </div>
      </div>
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    curYearMonth () {
      ...
      return dateTimeFormatter(tempDate, this.kor[this.calendar.options.locale].format)
    },
    customColor () {
